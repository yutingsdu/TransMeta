		...
		{ // DEBUG ONLY
			...
			for(int i=0;i<keeptrf.Count();i++) {
				...
				while(j<transfrag[keeptrf[i].t]->nodes.Count()) {
					fprintf(stderr,"Ad5\tTRANSMETA\texon\t%d",no2gnode[transfrag[keeptrf[i].t]->nodes[j]]->start);
	...
	*/
	...
	for(int n=0;n<npred;n++) if(pred[n]->flag){
		...
		*/
		...
		fprintf(f_out,"%s\tTRANSMETA\ttranscript\t%d\t%d\t1000\t%c\t.\tgene_id \"%s\"; transcript_id \"%s\"; ",
		...
		for(int j=0;j<pred[n]->exons.Count();j++) {
			fprintf(f_out,"%s\tTRANSMETA\texon\t%d\t%d\t1000\t%c\t.\tgene_id \"%s\"; transcript_id \"%s\"; exon_number \"%d\"; ",
		...
		if(keep[n]){
			...
			while(m<npred && pred[m]->start<=currentend) {
				if(keep[m] && pred[m]->geneno ==pred[n]->geneno) { // this is a prediction worth printing
					...
					fprintf(f_out,"%s\tTRANSMETA\ttranscript\t%d\t%d\t1000\t%c\t.\tgene_id \"%s.%d\"; transcript_id \"%s\"; ",
					...
					for(int j=0;j<pred[m]->exons.Count();j++) {
						fprintf(f_out,"%s\tTRANSMETA\texon\t%d\t%d\t1000\t%c\t.\tgene_id \"%s.%d\"; transcript_id \"%s\"; exon_number \"%d\"; ",
